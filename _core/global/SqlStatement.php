...
class UserAuthSqlStatement {
        ...
        const GET_ALL = 'SELECT p.surname, p.firstname, p.middlename, p.userid, p.local_address, p.home_address, p.telephone, p.sex, p.birth_date, ua.usertype, ua.regNo FROM profile as p LEFT JOIN user_auth as ua ON (p.userid = ua.userid)';
        ...
        const GET_USER_BY_CREDENTIALS = "SELECT u.userid, u.regNo, u.usertype, u.status, u.online_status, p.profile_id, p.surname, p.firstname, p.middlename, p.department_id, p.sex
            FROM user_auth AS u
                LEFT JOIN PROFILE AS p
                    ON(u.userid=p.userid)
            WHERE regNo=:regNo AND passcode=SHA1(:passcode)";

        const GET_USER_BY_ID = 'SELECT u.userid, u.regNo, u.usertype, u.online_status, p.profile_id, p.surname, p.firstname, p.middlename, p.department_id, p.sex
        FROM user_auth AS u
            LEFT JOIN PROFILE AS p
                ON(u.userid=p.userid)
