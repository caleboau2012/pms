...
class UserAuthSqlStatement {
    ...
    const GET_ALL = 'SELECT p.surname, p.firstname, p.middlename, ua.userid, p.department_id, p.work_address, p.home_address, p.telephone, p.sex, p.birth_date, ua.regNo, ua.active_fg FROM profile as p RIGHT JOIN user_auth as ua ON (p.userid = ua.userid)';
...
class RosterSqlStatement {
    ...
    const PERIOD_AVAILABLE = 'SELECT COUNT(*) as count FROM roster AS r WHERE (r.active_fg = 1) AND r.duty_date = :duty_date AND r.duty = :duty AND r.user_id = :user_id';
...
class VitalsSqlStatement {
    const ADD = "INSERT INTO vitals (patient_id, added_by, temp, pulse, respiratory_rate, blood_pressure, height, weight, bmi, active_fg, created_date) VALUES (:patient_id, :added_by, :temp, :pulse, :respiratory_rate, :blood_pressure, :height, :weight, :bmi, 1, NOW())";
...
class UrineSensitivitySqlStatement {
    ...
    const GET    = 'SELECT ur.isolates, ur.isolates_degree
                        ...
                        WHERE ur.urine_id IN (SELECT urine_id FROM urine WHERE treatment_id = :treatment_id AND encounter_id = :encounter_id)
...
class ChemicalPathologyRequestSqlStatement2 {
    ...
    const GET_ALL_TEST = 'SELECT p.surname, p.middlename, p.firstname, p.regNo, cpr.status_id, cpr.treatment_id AS testid, cpr.modified_date, cpr.cpreq_id
                            ...
                            LEFT JOIN patient p ON (p.patient_id=t.patient_id)
                            ORDER BY cpr.created_date DESC';
...
class ParasitologyRequestSqlStatement {
    const ADD_REQ_INFO = "INSERT INTO parasitology_req (doctor_id, treatment_id, encounter_id, diagnosis, created_date, modified_date)
                          VALUES (:doctor_id, :treatment_id, :encounter_id, :diagnosis, NOW(), NOW())";
    const GET_HISTORY = "SELECT preq_id as testId, diagnosis as diagnosis, modified_date as modified_date, status_id
                         as status, treatment_id as treatment_id, encounter_id as encounter_id FROM parasitology_req WHERE treatment_id IN
...
class EncounterSqlStatement{
    ...
    const UPDATE = "UPDATE encounter SET personnel_id = :personnel_id, consultation = :consultation, symptoms = :symptoms,
                    diagnosis = :diagnosis, bill_status = 1, comments = :comments, modified_date = NOW() WHERE treatment_id = :treatment_id AND patient_id = :patient_id
...
class RadiologyRequestSqlStatement{
    const ADD_RAD_INFO = "INSERT INTO radiology (doctor_id, treatment_id, encounter_id, created_date, modified_date)
    ...
    const GET_HISTORY = "SELECT rr.	radiology_request_id as testId,rr.clinical_diagnosis_details as diagnosis, r.modified_date as modified_date, r.status_id
                         as status, r.treatment_id as treatment_id, r.encounter_id as encounter_id FROM radiology AS r INNER JOIN radiology_request AS rr
    ...
    const GET_PATIENT_QUEUE = "SELECT * FROM radiology_request AS rr INNER JOIN radiology AS r ON
                               ...
                               WHERE r.status_id = :status_id AND rr.active_fg = :active_fg GROUP By r.treatment_id, r.encounter_id
...
class HaematologyRequestSqlStatement{
    const ADD_REQ_INFO = "INSERT INTO haematology (doctor_id, treatment_id, encounter_id, clinical_diagnosis_details, created_date, modified_date)
                          VALUES (:doctor_id, :treatment_id, :encounter_id, :clinical_diagnosis_details, NOW(), NOW())";
    const GET_HISTORY = "SELECT h.haematology_id as testId, h.clinical_diagnosis_details as diagnosis, h.modified_date as modified_date, h.status_id
                         as status, h.treatment_id as treatment_id, h.encounter_id as encounter_id FROM haematology AS h WHERE treatment_id IN
                         (SELECT treatment_id FROM treatment AS t WHERE t.patient_id = :patient_id)";
...
class FilmAppearanceSqlStatement {
    ...
    const GET    = 'SELECT film_appearance_id, haematology_id,aniscocytosis,poikilocytosis,polychromasia,macrocytosis,microcytosis,hypochromia,sickle_cells,target_cells,spherocytes,nucleated_rbc,sickling_test,created_date,modified_date FROM film_appearance f WHERE haematology_id = :haematology_id ORDER BY f.created_date DESC LIMIT 1';
...
class DifferentialCountSqlStatement {
    ...
    const GET    = 'SELECT * FROM differential_count d WHERE d.haematology_id = :haematology_id ORDER BY d.created_date DESC LIMIT 1';
...
class MicroscopyRequestSqlStatment{
    const ADD_REQ_INFO = "INSERT INTO urine (doctor_id, treatment_id, encounter_id, clinical_diagnosis_details, created_date, modified_date)
                          VALUES (:doctor_id, :treatment_id, :encounter_id, :clinical_diagnosis_details, NOW(), NOW())";
    const GET_HISTORY = "SELECT urine_id as testId, clinical_diagnosis_details as diagnosis, modified_date as modified_date, status_id
                         as status, treatment_id as treatment_id, encounter_id as encounter_id FROM urine WHERE treatment_id IN (SELECT treatment_id FROM treatment AS t WHERE t.patient_id = :patient_id)";
...
class ChemicalPathologyRequestSqlStatement{
    const ADD_REQ_INFO = "INSERT INTO chemical_pathology_request (doctor_id, treatment_id, encounter_id, clinical_diagnosis, created_date, modified_date)
                          VALUES (:doctor_id, :treatment_id, :encounter_id, :clinical_diagnosis, NOW(), NOW())";
    const GET_HISTORY = "SELECT cpreq_id as testId, clinical_diagnosis as diagnosis, modified_date as modified_date, status_id
                         as status, treatment_id as treatment_id, encounter_id as encounter_id FROM chemical_pathology_request WHERE treatment_id IN
                         (SELECT treatment_id FROM treatment AS t WHERE t.patient_id = :patient_id)";
    ...
    const GET_VALUES = "SELECT cpref_id, result FROM chemical_pathology_details WHERE cpreq_id =:cpreq_id";
...
//    VALUES(:doctor_id, :patient_id, NOW(), NOW(), 1)";
    ...
    const GET_PROCEDURE = "SELECT treatment_id, consultation, symptoms, diagnosis, comments FROM treatment WHERE treatment_id = :treatment_id AND encounter_id = 0 AND bill_status = 1 LIMIT 1";
 ...
 FROM haematology INNER JOIN treatment ON (treatment.treatment_id = haematology.treatment_id)
                            INNER JOIN patient_queue ON (patient_queue.patient_id = treatment.patient_id)
                             WHERE haematology.treatment_id = :treatment_id AND patient_queue.active_fg = 0 AND haematology.encounter_id = 0
 ...
 FROM haematology INNER JOIN encounter ON (encounter.encounter_id = haematology.encounter_id)
    ...
    const URINETEST = "SELECT DISTINCT urine_id, clinical_diagnosis_details, urine.created_date
 FROM urine INNER JOIN treatment ON (treatment.treatment_id = urine.treatment_id)
                            INNER JOIN patient_queue ON (patient_queue.patient_id = treatment.patient_id)
                             WHERE urine.treatment_id = :treatment_id AND patient_queue.active_fg = 0 AND urine.encounter_id = 0
 ...
 FROM urine INNER JOIN encounter ON (encounter.encounter_id = urine.encounter_id)
    ...
    FROM visual_skills_profile INNER JOIN treatment ON (treatment.treatment_id = visual_skills_profile.treatment_id)
                            INNER JOIN patient_queue ON (patient_queue.patient_id = treatment.patient_id)
                            WHERE visual_skills_profile.treatment_id = :treatment_id AND patient_queue.active_fg = 0 AND visual_skills_profile.encounter_id = 0
                            ORDER BY visual_skills_profile.created_date DESC";

    const VISUALTEST_BY_ENCOUNTER = "SELECT DISTINCT visual_profile_id, description, visual_skills_profile.created_date
    FROM visual_skills_profile INNER JOIN encounter ON (encounter.encounter_id = visual_skills_profile.encounter_id)
                            INNER JOIN patient_queue ON (patient_queue.patient_id = encounter.patient_id)
                            WHERE visual_skills_profile.encounter_id = :encounter_id AND patient_queue.active_fg = 0
                            ORDER BY visual_skills_profile.created_date DESC";

    const CHEMICALTEST = "SELECT DISTINCT cpreq_id, clinical_diagnosis, chemical_pathology_request.created_date
FROM chemical_pathology_request INNER JOIN treatment ON (treatment.treatment_id = chemical_pathology_request.treatment_id)
                            INNER JOIN patient_queue ON (patient_queue.patient_id = treatment.patient_id)
                            WHERE chemical_pathology_request.treatment_id = :treatment_id AND patient_queue.active_fg = 0 AND chemical_pathology_request.encounter_id = 0
                            ORDER BY chemical_pathology_request.created_date DESC";

    const CHEMICALTEST_BY_ENCOUNTER = "SELECT DISTINCT cpreq_id, clinical_diagnosis, chemical_pathology_request.created_date
FROM chemical_pathology_request INNER JOIN encounter ON (encounter.encounter_id = chemical_pathology_request.encounter_id)
                            INNER JOIN patient_queue ON (patient_queue.patient_id = encounter.patient_id)
                            WHERE chemical_pathology_request.encounter_id = :encounter_id AND patient_queue.active_fg = 0
                            ORDER BY chemical_pathology_request.created_date DESC";

    const PARATEST = "SELECT DISTINCT preq_id, parasitology_req.diagnosis, parasitology_req.created_date
FROM parasitology_req INNER JOIN treatment ON (treatment.treatment_id = parasitology_req.treatment_id)
                            INNER JOIN patient_queue ON (patient_queue.patient_id = treatment.patient_id)
                            WHERE parasitology_req.treatment_id = :treatment_id AND patient_queue.active_fg = 0 AND parasitology_req.encounter_id = 0
                            ORDER BY parasitology_req.created_date DESC";

    const PARATEST_BY_ENCOUNTER = "SELECT DISTINCT preq_id, parasitology_req.diagnosis, parasitology_req.created_date
FROM parasitology_req INNER JOIN encounter ON (encounter.encounter_id = parasitology_req.encounter_id)
                            INNER JOIN patient_queue ON (patient_queue.patient_id = encounter.patient_id)
                            WHERE parasitology_req.encounter_id = :encounter_id AND patient_queue.active_fg = 0
                            ORDER BY parasitology_req.created_date DESC";
...
FROM radiology INNER JOIN treatment ON (treatment.treatment_id = radiology.treatment_id)
                            INNER JOIN patient_queue ON (patient_queue.patient_id = treatment.patient_id)
                            WHERE radiology.treatment_id = :treatment_id AND patient_queue.active_fg = 0 AND radiology.encounter_id = 0
...
class ReportSqlStatement {
    ...
    //  Number  list and graph of new (male and female)patients from start date to end date
    ...
    // A graphical representation of patient age against diagnosis
    const PATIENT_AGE_AGAINST_DIAGNOSIS = "SELECT CONCAT(UPPER(p.surname), ' ', p.middlename, ' ', p.firstname) AS patient_name, t.created_date
                                                DATE_FORMAT(FROM_DAYS(DATEDIFF(DATE(NOW()), birth_date)), '%Y')+0 AS age, t.diagnosis, t.created_date AS consultation_date FROM treatment AS t
                                                LEFT JOIN patient AS p
                                                ON t.patient_id = p.patient_id
                                                WHERE DATE(t.created_date) BETWEEN DATE(:start_date) AND DATE(:end_date)";
